...
def check_horizontal(grid):
    ans = max(digit_product(grid[i][j: j + 4]) for i in range(len(grid))
              for j in range(len(grid) - 3))
    return ans


def check_vertical(grid):
    ans = max(digit_product([grid[i][j], grid[i + 1][j], grid[i + 2][j],
                             grid[i + 3][j]]) for i in range(len(grid) - 3)
